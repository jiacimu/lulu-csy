...
function getModuleDictionarySummary(module: DateStatusModuleDefinition): string {
    return module.description.split(/[：:]/)[0]?.trim() || module.name;
}

function createStatusFieldDictionary(modules: DateStatusModuleDefinition[]): string {
    const lines: string[] = ['### 四、模块与字段(v2;每条标〔类型|层级〕)'];

    for (const module of modules) {
        lines.push('');
        lines.push(`**【${module.name}】** ${getModuleDictionarySummary(module)}`);
        for (const fieldDef of module.fields) {
            const name = getTemplateFieldName(module, fieldDef);
            lines.push(`- ${name} 〔${getTypeHint(fieldDef)}|${fieldDef.level === 'display' ? '展示' : '内核'}〕${fieldDef.effect}`);
        }
    }
...
function createFieldContract(modules: DateStatusModuleDefinition[]): string {
    return createStatusFieldDictionary(modules);
}
...
function coreDetails(_module: DateStatusModuleDefinition, rows: string, className = 'core'): string {
