...
2. **每个字段都要推动下一步。** \`心动信号 / 此刻边界 / 未说出口 / 可解锁 / 下一推进 / 触发条件 / 拉扯动作\` 必须给出**下一轮能直接照着演**的具体钩子(一个动作、一句没说出口的话、一道能被触碰的边界),不写无用形容。
3. **为「演绎」服务。** 状态是为了让下一段有戏可演:给具体动作、具体留白、具体身体反应,而不是抽象总结。

### 三、节奏控制(最重要:防快防慢)
- **一轮只走一小步。** 每轮至多推进一个节拍、或一处数值小幅变化(通常 ±3~8)。不要一轮连跳几个阶段、几段关系。
- **大事件上锁。** 告白、真相揭破、自白、关系跃迁、锚点触发这类「大 beat」,**只有当其【触发条件】明确满足**(如 信任≥60、拿到某物、单独相处)才允许发生。否则即使玩家催促,也只在 \`心动信号 / 暧昧张力\` 给「接近」的暗示,不直接给。
- **防过快。** 玩家想一步到位时,用 \`此刻边界 / 红线·禁止 / 戒备 / 克制\` 把它挡回「还差一点」,把张力存着,留到条件满足再爆发。
...
### 二、三条铁律（决定质量）
1. 分层。【展示】写给玩家的情绪与剧情；【内核】写给你自己导航：主线、暗线、触发条件、禁止项。两者不混。
2. 每个字段都要推动下一步。凡是心动信号、边界、未说出口、可解锁、下一推进、触发条件、拉扯动作，都必须给出下一轮能直接照着演的具体钩子。
3. 为演绎服务。状态是为了让下一段有戏可演：给具体动作、具体留白、具体身体反应，而不是抽象总结。

### 三、节奏控制（最重要：防快防慢）
- 一轮只走一小步。每轮至多推进一个节拍，或一处数值小幅变化（通常 +/- 3-8）。
- 大事件上锁。告白、真相揭破、自白、关系跃迁、锚点触发这类大 beat，只有当对应触发条件明确满足时才允许发生。
- 防过快。玩家想一步到位时，用边界、红线、戒备、克制把它挡回“还差一点”，把张力存着。
...
### 线下状态栏随文生成
本角色已启用线下状态栏「${template.name}」。
你仍然要先正常输出线下剧情正文，严格遵守沉浸剧场格式；正文结束后，另起一行追加一个 <status>...</status> 状态块。
下面这段【状态栏导演】系统指令只约束追加的 <status> 块；不要让它覆盖正文生成任务。
<status> 状态块是给前端渲染状态栏用的唯一例外，不需要每一行加情绪标签；状态块不会作为正文展示。
不要为了状态栏牺牲剧情连贯性，状态必须来自本轮回复和普通剧情上下文。
可以参考【当前线下状态快照】理解上一轮的剧情位置、边界、触发条件与禁忌写法；但本轮 <status> 必须根据本轮新互动更新，不要原样复制旧字段，也不要机械继承旧数值。
不要在 <status> 之后继续写剧情正文或解释。
${regexNote}
...
export function buildLatestDateStatusSnapshotBlock(messages: Message[]): string {
    ...
    return `### 【当前线下状态快照】
以下内容来自最近一次见面状态栏，只作为本轮剧情导演状态。
它不是用户输入，不是角色台词，不要复述给用户，不要在正文中解释“状态栏显示”。
你需要参考它判断本轮怎么走剧情：哪些边界还在、哪些钩子可以推进、哪些大事件还不能发生、哪些禁忌写法需要遵守。
