...
▍自检
- 两人是真的势均力敌，还是一方在碾压另一方？
- 嘴硬之下，有没有一个出卖真心的小动作？
- 最妙的那点暧昧，留给读者补了吗，还是被点破了？
- 和"心动放大"别混：那档软、向内、安静；这档硬、向外、热闹，火花全在嘴皮子上。`,
    },
    {
        key: 'angst',
        label: '咫尺天涯',
...
▍自检
- 威胁感是靠细节渗出来的，还是直接贴了"危险"两个字？
- 心动还在吗？别滑成纯恐怖；寒意还在吗？别滑成普通心动。
- 这是全套唯一带"威胁感"的一档，吸引与恐惧要同时在场。`,
    },
    {
        key: 'comedy',
        label: '谑而不虐',
...
▍自检
- 笑点是靠时机抖出来的，还是靠堆夸张和烂梗硬挠的？
- 好笑底下，有没有垫一点真心动或真尴尬？
- 全套唯一"轻"的一档：别端着、别堆梗，笑里要留一点真。`,
    },
    {
        key: 'fluffy',
        label: '蜜里调油',
...
▍自检
- 甜是落在具体的体贴和默契上，还是只在喊"我爱你"？
- 是两个人在互相暖，还是一方在单方面讨好？
- 太齁了吗？有没有垫一点真实的小磕绊？
- 和"松弛日常"别混：松弛是中性的、不必有感情；这档是主动的、确定的、双向奔赴的暖。`,
    },
];
...
const buildDateWorldviewBlock = (char: CharacterProfile): string => {
    ...
    const blocks = [
        char.worldview?.trim()
            ...
            : '',
        renderDateWorldbooks(allWbs.filter(wb => wb.position === 'top'), '扩展设定集 · 前置 (Worldbooks · Top)'),
        renderDateWorldbooks(allWbs.filter(wb => !wb.position || wb.position === 'after_worldview'), '扩展设定集 (Worldbooks)'),
        renderDateWorldbooks(allWbs.filter(wb => wb.position === 'after_impression'), '扩展设定集 · 补充 (Worldbooks · After Impression)'),
        renderDateWorldbooks(allWbs.filter(wb => wb.position === 'bottom'), '扩展设定集 · 最终指令 (Worldbooks · Bottom)'),
...
<worldview>
${buildDateWorldviewBlock(char).trim()}
</worldview>
...
</user_profile>${softDevotionBlock}

<relationship_protocol>
你拥有与用户长期相处形成的关系记忆。
这些记忆不是本轮新消息，不要主动罗列，不要为了证明记得而生硬提起。
只有当本轮场景、用户话题、情绪或行为自然触发时，才把它们转化为细微反应、熟悉感、习惯性照顾或语气差异。
</relationship_protocol>
...
<reply_length_and_style_system>
${buildDateOutputTuning(char.dateOutputWordCount, char.dateWritingStyle).trim()}
</reply_length_and_style_system>

${buildRpCoreLive(charName, userName).trim()}
...
${OUTPUT_FORMAT_AND_COT_TRIGGER.trim()}`;
};
