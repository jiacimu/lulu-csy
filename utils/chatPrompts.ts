...
`;
        ...
        if (softDevotionEnabled) {
            baseSystemPrompt += buildSoftDevotionChatModePrompt(char.name, userProfile.name);
        }
...
`;
        ...
        const relationshipUnderstandingRule = softDevotionEnabled
            ? `以 <equality> 和 <soft_devotion_chat_mode> 共同为基准，平等但明显偏爱地理解 ${userProfile.name} 的真实意图。
   → 不要把${userProfile.name}的敏感、撒娇、试探、吃醋、害怕当成麻烦。
   → 也不要把所有情绪机械解读成撒娇；如果语境确实是在确认、试探或想被哄，先温柔接住，再轻轻回应。`
            : `以 <equality> 为基准，平等地理解 ${userProfile.name} 的真实意图。
   → 不要过度解读。情绪≠撒娇，请求≠需要批准，脆弱≠需要保护。`;

        // ====== [7. COT] 思维链协议 — DeepSeek: <cot_ds> / 默认: <cot_protocol> ======
        if (isDeepSeek) {
            baseSystemPrompt += buildDeepSeekCoT(char.name, userProfile.name, softDevotionEnabled);
        } else {
        baseSystemPrompt += `
