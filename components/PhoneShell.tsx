...
const PhoneShell: React.FC = () => {
  ...
  // ── PWA: Edge Swipe Trap to Hijack System Back Gesture ──────────────────────────
  // Prevents Android's native side swipe from throwing the browser out of Fullscreen API
  useEffect(() => {
    if (Capacitor.isNativePlatform()) return;
    ...
    const handleTouchStart = (e: TouchEvent) => {
      if (e.touches.length !== 1 || isLockedRef.current) return;
      const touch = e.touches[0];
      // Start trap if touch is within 25px of the left edge
      if (touch.clientX < 25) {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      // Lock horizontal motion to hijack system gesture
      if (deltaX > deltaY) {
        if (e.cancelable) e.preventDefault();

        // Trigger our custom back routing once pulled far enough
        if (deltaX > 60 && !swipeTriggered) {
          swipeTriggered = true;
          haptic.selection(); // Native "thud" feel
          
          // Use setTimeout so the UI transitions smoothly on next frame
          setTimeout(() => {
             handleBackRef.current();
          }, 0);
        }
      } else if (deltaY > 30) {
        isEdgeSwipe = false; // Moved vertically too much, release trap
      }
    };
